<div>
                ...
                <x-table.tbody>
                    @forelse($posts as $post)
                        <x-table.tr>
                            ...
                            <x-table.td>
                                @if ($post->status === \App\Models\Post::POST_ACITVE)
                                    <span class="badge text-white bg-green-500">{{ __('Active') }}</span>
                                @elseif($post->status === \App\Models\Post::POST_INACTIVE)
                                    <span class="badge text-white bg-red-500">{{ __('Inactive') }}</span>
                                @endif
                            </x-table.td>
                            <x-table.td>
                                <livewire:toggle-button :model="$post" field="status" key="{{ $post->id }}" />
                            </x-table.td>
                            <x-table.td>
                                <div class="inline-flex">
                                    ...
                                    <button class="btn btn-sm text-white bg-red-500 border-red-800 hover:bg-red-600 active:bg-red-700 focus:ring-red-300 " type="button"
                                        wire:click="confirm('delete', {{ $post->id }})"
    ...
    <form wire:submit.prevent="deleteSelected">
        <x-modal.confirmation wire:model.defer="showDeleteModal">
            <x-slot name="title">{{ __('Delete Selected Posts') }}</x-slot>
