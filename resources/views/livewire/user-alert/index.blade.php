                ...
                <x-table.tr>
                    ...
                    <x-table.td>
                        <div class="flex justify-center">
                            @can('user_alert_show')
                                <a class="btn btn-sm btn btn-sm text-white bg-green-500 border-green-800 hover:bg-green-600 active:bg-green-700 focus:ring-green-300mr-2"
                                    href="{{ route('admin.user-alerts.show', $userAlert) }}">
                                    <x-heroicon-o-eye class="h-4 w-4" />
                                </a>
                            @endcan
                            @can('user_alert_edit')
                                <a class="btn btn-sm btn btn-sm text-white bg-blue-500 border-blue-800 hover:bg-blue-600 active:bg-blue-700 focus:ring-blue-300 mr-2"
                            ...
                            @can('user_alert_delete')
                                <button class="btn btn-sm text-white bg-red-500 border-red-800 hover:bg-red-600 active:bg-red-700 focus:ring-red-300" type="button"
                                    wire:click="confirm('delete', {{ $userAlert->id }})" wire:loading.attr="disabled">
                                    <x-heroicon-o-trash class="h-4 w-4" />
                                </button>
                            @endcan
                        </div>
    ...
    <div class="p-4">
      ...
      <form wire:submit.prevent="deleteSelected">
        <x-modal.confirmation wire:model.defer="showDeleteModal">
            <x-slot name="title">{{ __('Delete Selected User Alerts') }}</x-slot>
            ...
            <x-slot name="footer">
                <button type="button" class="btn border-gray-300 text-gray-700 dark:text-gray-300 active:bg-gray-50 dark:active:text-gray-800 hover:text-gray-500 dark:active:bg-dark-eval-1 active:text-gray-300 dark:hover:text-gray-700" wire:click="$set('showDeleteModal', false)">{{ __('Go back') }}</button>
