...
class ProductCart extends Component
{
    ...
    public function calculate($product) {
        ...
        if ($product['product_tax_type'] == 1) {
            $price = $product['price'] + ($product['price'] * ($product['product_order_tax'] / 100));
            $unit_price = $product['price'];
            $product_tax = $product['price'] * ($product['product_order_tax'] / 100);
            $sub_total = $product['price'] + ($product['price'] * ($product['product_order_tax'] / 100));
        } elseif ($product['product_tax_type'] == 2) {
            $price = $product['price'];
            $unit_price = $product['price'] - ($product['price'] * ($product['product_order_tax'] / 100));
            $product_tax = $product['price'] * ($product['product_order_tax'] / 100);
