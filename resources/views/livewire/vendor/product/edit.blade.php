<div>
    ...
    <form enctype="multipart/form-data" wire:submit.prevent="submit" class="pt-3">

        <div class="flex flex-wrap -m-2">
            <div class="lg:w-1/3 md:w-1/2 sm:w-full p-2 {{ $errors->has('product.code') ? 'is-invalid' : '' }}">
                <x-label for="product.code" :value="__('Code')" required />
                <input
                    class="p-3 leading-5 bg-white dark:bg-dark-eval-2 text-gray-700 dark:text-gray-300 rounded border border-gray-300 mb-1 text-sm w-full focus:shadow-outline-blue focus:border-blue-500"
                    type="text" name="product.code" id="product.code" required wire:model.defer="product.code">
                <x-input-error for="product.code" />
            </div>
            <div class="lg:w-1/3 md:w-1/2 sm:w-full p-2 {{ $errors->has('product.category') ? 'is-invalid' : '' }}">
                <x-label for="product.category" :value="__('Category')" />
            ...
            <div class="lg:w-1/3 md:w-1/2 sm:w-full p-2 {{ $errors->has('product.name') ? 'is-invalid' : '' }}">
                <x-label for="product.name" :value="__('Name')" required />
                <input
                    class="p-3 leading-5 bg-white dark:bg-dark-eval-2 text-gray-700 dark:text-gray-300 rounded border border-gray-300 mb-1 text-sm w-full focus:shadow-outline-blue focus:border-blue-500"
                    type="text" name="product.name" id="product.name" required wire:model.defer="product.name">
                <x-input-error for="product.name" />
            </div>
            <div class="lg:w-1/3 md:w-1/2 sm:w-full p-2 {{ $errors->has('product.') ? 'is-invalid' : '' }}">
                <x-label for="product.price" :value="__('Price')" required />
                <input
                    class="p-3 leading-5 bg-white dark:bg-dark-eval-2 text-gray-700 dark:text-gray-300 rounded border border-gray-300 mb-1 text-sm w-full focus:shadow-outline-blue focus:border-blue-500"
                    type="number" name="product.price" id="product.price" required wire:model="product.price">
                <x-input-error for="product.price" />
            </div>
            <div class="lg:w-1/3 md:w-1/2 sm:w-full p-2">
                <x-label for="product.wholesale_price" :value="__('Wholesale Price')" required />
                <input
                    class="p-3 leading-5 bg-white dark:bg-dark-eval-2 text-gray-700 dark:text-gray-300 rounded border border-gray-300 mb-1 text-sm w-full focus:shadow-outline-blue focus:border-blue-500"
        ...
        <div class="mb-4 p-2 {{ $errors->has('product.description') ? 'is-invalid' : '' }}">
            <x-label for="product.description" :value="__('Description')" />
            <input
                class="p-3 leading-5 bg-white dark:bg-dark-eval-2 text-gray-700 dark:text-gray-300 rounded border border-gray-300 mb-1 text-sm w-full focus:shadow-outline-blue focus:border-blue-500"
                type="text" name="product.description" id="product.description" wire:model.defer="product.description">
